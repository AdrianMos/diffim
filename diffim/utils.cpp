...
void print_app_title()
{
    ...
    print_separator(width, ' ');
    print_centered("open-source batch image comparer, with masks support", width,
                   ' ');
    print_centered("V 2.0, 07.2020", width, ' ');
    print_separator(width, '*');
    std::cout << "\n";
}
...
void create_output_folders(const Parameters& p)
{
	...
	if (!fs::create_directory(p.diff_folder))
...
{
	try {
		fs::create_directory(p.reference_folder);
		fs::create_directory(p.compare_folder);
		fs::create_directory(p.masks_folder);
	}
	catch (...) {
